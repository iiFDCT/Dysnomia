...
class Member extends Base {
    ...
    /**
     * The server nickname of the member, their global name, or their username if they don't have either set
     * @type {String}
     */
    get displayName() {
    ...
    /**
     * The ID of the members' highest role
     * @type {String}
     */
    get highestRole() {
        const roles = this.roles.filter((r) => this.guild.roles.has(r));
        return roles.length == 0 ? this.guild.id : roles.map((r) => this.guild.roles.get(r)).sort((a, b) => b.position - a.position)[0].id;
    }
    ...
    /**
     * The users username with an "@" at the start, or the users username and discriminator (for bots, or users that aren't yet using the usernames system)
     * @type {String}
     */
    get tag() {

...
class Client extends EventEmitter {
    ...
    * @arg {String} [reason] The reason to be displayed in audit logs
    * @returns {Promise<Emoji>} A guild emoji object
    */
    createGuildEmoji(guildID, options, reason) {
        options.reason = reason;
        return this.requestHandler.request("POST", Endpoints.GUILD_EMOJIS(guildID), true, options).then((emoji) => new Emoji(emoji, this.guilds.get(guildID), this));
    }
    ...
    */
    editGuildEmoji(guildID, emojiID, options, reason) {
        options.reason = reason;
        return this.requestHandler.request("PATCH", Endpoints.GUILD_EMOJI(guildID, emojiID), true, options).then((emoji) => new Emoji(emoji, this.guilds.get(guildID), this));
    }
    ...
    * @arg {String?} [options.channel_id] The channel ID for the guild widget
    * @arg {String?} [options.reason] The reason to be displayed in audit logs
    ...
    * @arg {String} guildID The ID of the guild
    * @arg {String} emojiID The ID of the emoji
    * @returns {Promise<Emoji>} An emoji object
    */
    getRESTGuildEmoji(guildID, emojiID) {
        if(!this.options.restMode) {
            return Promise.reject(new Error("Dysnomia REST mode is not enabled"));
        }
        return this.requestHandler.request("GET", Endpoints.GUILD_EMOJI(guildID, emojiID), true).then((emoji) => new Emoji(emoji, null, this));
    }

    /**
    * Get a guild's emojis via the REST API. REST mode is required to use this endpoint.
    * @arg {String} guildID The ID of the guild
    * @returns {Promise<Array<Emoji>>} An array of guild emoji objects
    */
    getRESTGuildEmojis(guildID) {
        if(!this.options.restMode) {
            return Promise.reject(new Error("Dysnomia REST mode is not enabled"));
        }
        return this.requestHandler.request("GET", Endpoints.GUILD_EMOJIS(guildID), true).then((emojis) => emojis.map((emoji) => new Emoji(emoji, null, this)));
    }
